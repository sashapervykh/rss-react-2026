...
export const handlers = [
    ...
    http.get(`${API_ROUTES.BASE_URL}/${API_ROUTES.SEARCH}`, async ({ request }) => {
        ...
        switch (true) {
            ...
            case (query === TEST_REQUESTS.SERVER_ERROR): {
                return new HttpResponse(JSON.stringify({ status_message: 'Server Error' }), { status: 500 });
            }
            case (query === TEST_REQUESTS.UNKNOWN_ERROR): {
                return new HttpResponse(JSON.stringify({ status: 'error' }), { status: 404 });
            }
            default: {
                return HttpResponse.json({
