...
describe('MoviesList', () => {
  it('renders spinner when loading is true', () => {
    render(<MoviesList movies={[]} loading={true} errorMessage={null} />);

    expect(screen.getByRole('status')).toBeInTheDocument();
  });

  it('displays "no results" message when data array is empty', () => {
    render(<MoviesList movies={[]} loading={false} errorMessage={null} />);
  ...
  it('renders correct number of items when data is provided', () => {
    const { getAllByTestId } = render(
      <MoviesList movies={mockedMovies} loading={false} errorMessage={null} />
    );
  ...
  it('throws error when errorMessage exists and is handled by ErrorBoundary', () => {
    ...
    render(
      <ErrorBoundary>
        <MoviesList movies={[]} loading={false} errorMessage="API failed" />
      </ErrorBoundary>
    );
